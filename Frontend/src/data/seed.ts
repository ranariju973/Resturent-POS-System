...
export const NAV: NavEntry[] = [
  ...
  },
  {
    ...
    label: 'Employees',
    short: 'Staff',
    // `users` is already the Customers icon. Reusing it is a small duplication,
    // but src/icons/data.ts is an OFFLINE bundle — a name that is not in it
    // renders nothing at all, which is a worse outcome than a repeated glyph.
    icon: 'lucide:users',
    group: 'mgmt',
