...
export function Reports() {
  ...
  // The month's own split. This used to borrow `payTotal` from the DAILY
  // report, which meant the monthly percentages were computed against one
  // day's takings whenever they were computed at all.
  const monthPayTotal = (monthly?.byPaymentMethod ?? []).reduce((sum, p) => sum + p.total, 0);
      ...
      >
        ...
        {tab === 'daily' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <PaymentMethods rows={daily?.byPaymentMethod ?? []} total={payTotal} />
          </div>
        ) : null}
        ...
        {tab === 'monthly' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <PaymentMethods rows={monthly?.byPaymentMethod ?? []} total={monthPayTotal} />
          </div>
        ) : null}
  ...
  return (
    <div
      style={{
        ...
        flexDirection: 'column',
        gap: isMobile ? 10 : 16,
        // Lets the tile shrink to its column instead of shouldering into the
        // one next to it — a grid item is min-width:auto by default.
        minWidth: 0,
      }}
    >
      <span
      ...
      >
        <Icon icon={icon} size={isMobile ? 17 : 20} />
      </span>
...
/**
 * How the takings split across cash, card and UPI.
 *
 * Takes its rows and its total as props rather than reading the store, because
 * both the daily and the monthly tab render one — and the bug this fixes was
 * exactly that: a single hardcoded copy sat in the monthly block while reading
 * the DAILY report, so it showed one day's figures under a monthly heading, or
 * nothing at all if the daily tab had not been opened first.
 */
    ...
    <ChartSection title="Payment methods">
      {rows.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {rows.map(({ method, total: amt }) => {
            const color = PAY_COLOR[method] ?? '#888780';
            const pct = total ? Math.round((amt / total) * 100) : 0;
            return (
              <div key={method} style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                <span
                  style={{
                    display: 'flex',
                    alignItems: 'baseline',
...
const statGrid: CSSProperties = {
  display: 'grid',
  gridTemplateColumns: 'repeat(auto-fit, minmax(clamp(140px, 45%, 210px), 1fr))',
  gap: 16,
};
